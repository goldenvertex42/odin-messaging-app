...
async function run() {
  ...
  for (const row of usersData) {
    ...
    userIds[res.rows[0].username] = res.rows[0].id;
  }
  ...
  const groupChatRes = await client.query(
    `INSERT INTO "conversation" (id, name, "isGroup", "createdAt", "updatedAt") 
     VALUES (gen_random_uuid(), 'Odin Developers Study Group', true, NOW(), NOW()) RETURNING id;`
  );
